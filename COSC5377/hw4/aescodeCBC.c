...
#include <sys/types.h>
#include <strings.h>
    unsigned char data[16];     // input buffer

/* Testing AES for a block */
int main( void )
{
	...
	unsigned char dat1[16];
    unsigned char data1[16];     // input buffer    unsigned char ciphertext[16];
    unsigned char plaintext[16];
    unsigned char ciphertext[16];

    unsigned char key[16] = {0x23, 0x5B, 0xAB, 0xBC, 0x94, 0x2A, 0x33, 0x8F,
                             0xA1, 0xBB, 0x26, 0x9A, 0x45, 0x63, 0x04, 0x50};       // 128-bit key
    ...
    if (fp2 == NULL)
    {
       printf("Failed to open output file: crypt.dat\n");
       return 0;
    }  
    ...
    /* Read only one block */
    nbytes = fread(data, 1, 16, fp1);
	for (i = 0; i < 16; i++)
		ciphertext[i] = data[i] ^ iv[i];
	/*aes_encrypt(&ctx, data, ciphertext);*/
	fwrite(ciphertext, 1, 16, fp2);

    int count =0, count1;
    while (nbytes >0)
    {
	count++;
	if (nbytes < 16) // padding
	{
		count1 = 16 - nbytes;
		printf ("nbytes is %d < 16, which need padding!\n", nbytes);
			data[nbytes] = 1;
		for (i = nbytes+1; i < 15; i++)
			data[i] = 0;
		data[15] = 16-nbytes;
	} 
	aes_encrypt(&ctx, data, ciphertext);
	...
	int k;
	for(k = 0; k < 16; k++)
		dat[k] = data[k] ^ ciphertext[k];
	fwrite(dat, 1, 16, fp2);
    }

    fclose (fp2);

    fp4 = fopen("ciphertext.dat", "rb");
	if (fp4 == NULL)
    {
       printf("Failed to open output file: crypt.dat\n");
       return 0;
    ...
    /* Write plaintext block in a file */
    int nbytes1, j;
	nbytes1 = fread(data1, 1, 16, fp4);
	for (i = 0; i < 16; i++)
		plaintext[i] = data1[i] ^ iv[i];
	/*aes_decrypt(&ctx, data1, plaintext);*/
	fwrite(plaintext, 1, 16, fp3);
    for (j = 1; j < count - 1 ; j++)
		{
			nbytes1 = fread(data1, 1, 16, fp4);
			...
			int k;
			for(k = 0; k < 16; k++)
		          dat1[k] = plaintext[k] ^ data1[k];
			fwrite(dat1, 1, 16, fp3);
		}
	//read last block
	fread(data1, 1, 16, fp4);
	aes_decrypt(&ctx, data1, plaintext);
	int k;
	for(k = 0; k < 15; k++)
		          dat1[k] = dat[k] ^ ciphertext[k];
	fwrite(dat1, 1, 16- count1 , fp3);
