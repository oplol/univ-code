...
int main ()
{
  ...
  FILE *send_fp, *fp1;
  char filename[100] = "serv.dat";   /* filename to send */
  char filename1[100] = "cli_copy.dat";   /* filename to receive */
  int i;
  ...
  sa_serv.sin_port        = htons (7778);          /* Server Port number 
*/
 ...
 //printf("\nRead next %d bytes", err);
  CHK_SSL(err);
  }
  fclose(fp1);

  printf("\nIn server. Just after the first read\n");

  printf ("\nFile \"%s\" has been successfully copied from the client!\n", filename1);
  
   /* */
  int nbytes;
  send_fp=fopen(filename,"rb");
  nbytes = fread(buf,1,sizeof(buf), send_fp);
  while (nbytes)
  {
