...
FILE *OpenFile(int argc, char *argv[]) {
		...
		if (argc < 2 || strcmp(argv[1], "input1.txt") != 0) {
				fprintf(stderr, "usage: mpirun -np n(number of processes) aosproj3_1 input1.txt\n");
				fprintf(stderr, "invalid input file: %s\n", argv[1]);
				fprintf(stderr, "The default file \"input1.txt\" will be selected.\n\n");
				inputfile = fopen("input1.txt", "r");  // open the default file input1.txt to read
			} else {
					inputfile = fopen(argv[1], "r");  // open the file from command line to read
				}
		
		if (inputfile == NULL) {
		   printf("can't open input1.txt for reading. \n");
		   exit(0);
		}
