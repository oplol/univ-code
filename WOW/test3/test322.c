...
FILE *OpenFile(int argc, char *argv[]) {
		...
		if (argc < 2 || strcmp(argv[1], "input2.txt") != 0) {
				fprintf(stderr, "usage: mpirun -np n(number of processes) aosproj3_2 input2.txt\n");
				fprintf(stderr, "invalid input file: %s\n", argv[1]);
				fprintf(stderr, "The default file \"input2.txt\" will be selected.\n\n");
				inputfile = fopen("input2.txt", "r");  // open the default file input2.txt to read
			} else {
					inputfile = fopen(argv[1], "r");  // open the file from command line to read
				}
		
		if (inputfile == NULL) {
		   printf("can't open input2.txt for reading. \n");
		   exit(0);
		}
