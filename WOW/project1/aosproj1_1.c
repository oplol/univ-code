	...
	fp = fopen("aosproj1_1.txt", "w");  // open the file to write
     ...
     if (fp == NULL) {
	  ...
	  times = 100;
		...
		for (count1 = 0; count1 <= 256; count1++) {  //how many messages you want to pingpong
		
			message_size = 1024 * count1;  // message size is increase by 4K each time because of the float type
